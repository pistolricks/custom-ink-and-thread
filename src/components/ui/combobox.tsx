...
const ComboboxItem = <T extends ValidComponent = "li">(
    ...
    return (
        <ComboboxPrimitive.Item
            class={cn(
                "relative flex cursor-default select-none items-center justify-between rounded-sm px-2 py-1.5 text-sm outline-hidden data-disabled:pointer-events-none data-highlighted:bg-green-6 data-highlighted:text-green-11 data-disabled:opacity-50",
                local.class
            )}
            {...others}
        />
    ...
    return (
        <ComboboxPrimitive.Portal>
            <ComboboxPrimitive.Content
                class={cn(
                    "relative z-50 max-h-64 overflow-y-auto min-w-32  rounded-md border border-gray-dim bg-gray-subtle text-gray-dim shadow-md animate-in fade-in-80",
                    local.class
                )}
                {...others}
            >
