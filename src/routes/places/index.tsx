...
import {MagnifyingGlass, MapIcon} from "~/components/svg";
    ...
    return (
        ...
        <>
            <DrawerPrimitive contextId={'map1'} noOutsidePointerEvents={false} closeOnOutsidePointer={false}
                ...
                <GeoMap handleToggle={handleToggleGeolocation} toggleGeo={getToggleGeo()}
                        coordinates={getMyLocation()?.geometry?.coordinates} featureCollection={getStoreCollection}/>
                ...
                <FooterMenu childClass={'relative w-full sm:max-w-sm'}
                    ...
                    <DrawerPrimitive contextId={'map2'} noOutsidePointerEvents={false} closeOnOutsidePointer={false}
                                     breakPoints={[0.4]} side={getIsDesktop() ? 'right' : 'bottom'} defaultSnapPoint={1}
                                     snapPoints={[0, 1]}
                                     dialogId="responsive-drawer-mobile" open={getOpenForm()}
                        ...
                        <Show when={getToggleGeo()}>
                            <AddressSearchForm onClick={handleOpenForm} contextId={'map2'}
                                               class={getToggleGeo() ? 'w-full sm:max-w-sm' : ''}/>
                        </Show>
                    </DrawerPrimitive>
                    <Show when={!getToggleGeo()}>
